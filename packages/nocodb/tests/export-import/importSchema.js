...
async function createBaseTables() {
  ...
  for (let i = 0; i < ncIn.length; i++) {
    ...
    formula.push(
      ...tblSchema.columns.filter((a) => a.uidt === UITypes.Formula),
    );
...
async function createLinks() {
  ...
  for (let i = 0; i < link.length; i++) {
    if (
      ...
      dstTbl = await api.dbTable.readTable({ tableId: dstTbl.id });
...
function get_v2Id(v1ColId) {
  for (let i = 0; i < ncIn.length; i++) {
    ...
    if (
      undefined !==
      (colSchema = tblSchema.columns.find((x) => x.id === v1ColId))
    ) {
...
async function configureGrid() {
  ...
  for (let i = 0; i < ncIn.length; i++) {
    ...
    let srcTbl = await api.dbTable.readTable({ tableId: tblId });
